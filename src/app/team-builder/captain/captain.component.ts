...
import { Registration, RegistrationConfigService } from 'src/app/shared/services/registration-config.service';
import { StateService } from 'src/app/shared/services/state.service';
import * as _ from 'lodash';
import { CustomFieldDataService, CustomFieldsGroupType } from 'src/app/shared/services/custom-field.service';
...
export class CaptainComponent implements OnInit, OnChanges {
  ...
  error: 'NOT_FOUND' | 'NOT_OPEN' | 'VIP_CLOSED' | 'UNEXPECTED' | 'IS_FULL' | 'IS_CAPTAIN_ALREADY' | 'LOADING' = 'LOADING'
  ...
  ngOnInit() {
    ...
    this.RegistrationConfigDataService.getRegistrations(this.regConfigId, undefined, undefined, undefined, user.email).then(async (data: any) => {
      if (data && data.length > 0) {
        this.IS_CAPTAIN_ALREADY = true;
      }
      if (this.state === null || this.state.regConfigId !== this.regConfigId || !this.state.registrationData.classifications) {
        await this.setState(this.regConfigId, user, vipCode);
        // this.$state.go(this.initialState).then(() => this.setTransitionListener());

        console.log('this.get', this.stateService.getState('IRegistrationCaptainState'))
      } else {
        // this.error = null;
  ...
  setState(regConfigId: string, user: User, vipCode: string) {
    ...
    forkJoin([
      this.getClassifications(regConfigId), this.getRegistrationData(regConfigId, vipCode), this.getRegistrationConfig(regConfigId), this.getTeamPrices(regConfigId)
      // Add other observables as needed
    ]).subscribe(
      ([data1, data2, data3, data4]: any) => {
        console.log("🚀 ~ file: captain.component.ts:156 ~ CaptainComponent ~ setState ~ data4:", data4)
        console.log("🚀 ~ file: captain.component.ts:156 ~ CaptainComponent ~ setState ~ data3:", data3)
        console.log("🚀 ~ file: captain.component.ts:156 ~ CaptainComponent ~ setState ~ data2:", data2)
        console.log("🚀 ~ file: captain.component.ts:156 ~ CaptainComponent ~ setState ~ data1:", data1)
  ...
  getClassifications(regConfigId: string) {
    return this.TeamClassificationDataService.getTeamClassifications(regConfigId);
  }

  getRegistrationData(regConfigId: string, vipCode?: string) {
    return this.RegistrationConfigDataService.dataRegistrations(regConfigId, vipCode);
  }
  ...
  getRegistrationConfig(regConfigId: string) {
    return this.RegistrationConfigDataService.getRegistrationConfigById(regConfigId).then((result: any) => {
      ...
      return this.CustomFieldDataService.getCustomFieldsGroupByIds(forms).then((data: any) => {
        ...
        result.funnel.waiver[1] = _.find(data, (f) => f.type === CustomFieldsGroupType.Waiver && f.appliesToMinors);
        return result;
      });
    });
  }
