...
export class PaymentComponent implements OnInit {
  ...
  generateTeamNameRandom(capFirstName: any, capLastName: any) {
    return (
      capFirstName.split(' ')[0] + ' ' + capLastName.split(' ')[0] + "'s Team"
    );
  }
  ...
  affirmCheckoutProcess() {
    ...
    if (!this.state.team.name) {
      this.state.team.name = this.generateTeamNameRandom(
        this.state?.captain?.firstName,
        this.state.captain.lastName
      );
    }
    ...
    let order = PreAffirmOrder.fromJS({
      ...
      payment: Payment.fromJS({
        ...
        totalPrice:
          (this.state?.price?.subTotal ? this.state?.price?.subTotal : 0) +
          (this.state?.price?.fees ? this.state?.price?.fees : 0) +
          (this.state?.price?.taxes ? this.state?.price?.taxes : 0),
        paidPrice: this.state?.price?.total,
        stage: this.state?.price?.stage,
        cardNumber:
          this.cardData && this.cardData.cardNumber
            ? this.cardData.cardNumber.slice(-4)
            : '',
      }),
    });
    ...
    this.RegistrationConfigDataService.createPreAffirmOrder(
      this.state.regConfigId,
      order
    )
      .then((data: any) => {
        ...
        affirm.checkout({
          ...
          items: [
            {
              display_name: `${this.state.registrationConfig.raceYear} - ${
                this.state.registrationConfig.name
              } - ${this.state.team.type || ''} - Race Registration`,
        ...
        affirm.checkout.open({
          onFail: (e: any) => {
            ...
            this.RegistrationConfigDataService.updatePostAffirmOrder(
              this.state.regConfigId,
              order
            );
          },
          onSuccess: (a: any) => {
            ...
            if (this.state?.registrationConfig?.funnel?.form) {
              if (this.state.registrationConfig.funnel.form.customFields) {
                const customFields = [
                  ...this.state.registrationConfig.funnel.form.customFields,
                ];
            ...
            if (order?.captainData?.bornAt) {
              order.captainData.bornAt = this.DateService.getLocalISOTime(
                moment(order.captainData.bornAt).endOf('day').toDate()
              );
            }
            this.RegistrationConfigDataService.updatePostAffirmOrder(
              this.state.regConfigId,
  ...
  handleSuccessResponce(orderResponse: any, order: any) {
    ...
    if (this.state.captain.bornAt) {
      this.state.captain.bornAt = this.DateService.getLocalISOTime(
        moment(this.state.captain.bornAt).endOf('day').toDate()
      );
    }
    ...
    this.router.navigate([newStateName + '/confirmation']);
  ...
  addExpertVoicePixelScript(order: any) {
    ...
    script.innerHTML = `
          ...
          }',     // Discount code used on order, or discount group name. Used to identify commissionable orders.
          orderDiscount: '${purchaseDetails.price.discounts}',
          orderShipping: '${purchaseDetails.price.fees}',
          orderSubtotal: '${
            purchaseDetails.price.subTotal
          }',     // Order subtotal. After discount is applied, before tax & shipping. Commission calculated from this.
          orderTax: '${order.payment.taxAmount}',
          orderCurrency: 'USD',
          orderTotal: '${purchaseDetails.price.total}',
          products: [
              {id: '${
                this.state.registrationConfig.id
              }', // parent SKU - Should match product code in EV store file 
              name: '${this.state.registrationConfig.name}',
              sku: '${this.state.registrationConfig.id}', 
              upc: '', 
      ...
      lineItems: [
        {
          ...
          productName:
            this.state.registrationConfig.name +
            '  ' +
            this.state.registrationConfig.raceYear,
  ...
  next() {
    ...
    if (this.state.price?.total === 0) {
      ...
      result = this.AuthorizeNetService.authorize(this.cardData).then(
        (opaqueData: any) => {
  ...
  postOrder(opaqueData?: IOPaqueData) {
    ...
    return this.RegistrationConfigDataService.postOrder(
      this.state.regConfigId,
      order
    )
